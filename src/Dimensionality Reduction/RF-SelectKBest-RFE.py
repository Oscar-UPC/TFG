...
# Number of selected features to test in RFE
...
for n_features in n_features_to_select_list:
    ...
    print(f"\n=== Running RFE with {n_features} features (after SelectKBest prefilter) ===")

    # Directory for this configuration
    results_directory = os.path.join(directory, f'Results/RFE_prefilter/RFE_prefilter_{n_features}_features')
    ...
    for g in features:
        ...
        for train_ix, test_ix in outer_cv.split(X, y):
            ...
            # === 1) Pre-filter using SelectKBest ===
            skb = SelectKBest(score_func=f_classif, k=min(200, X_train.shape[1]))  # limit to 200 or less if fewer features
            ...
            # === 2) RFE on the pre-filtered features ===
            ...
            # === 3) Train final Random Forest on RFE-selected features ===
