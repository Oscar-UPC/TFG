...
for n_features in n_features_to_select_list:
    ...
    print(f"\n=== Running RFE (LR selector) + RF model with {n_features} features ===")

    results_directory = os.path.join(directory, f'Results/RFE_LR_RF/RFE_{n_features}_features')
    ...
    for g in features:
        ...
        for train_ix, test_ix in outer_cv.split(X, y):
            ...
            # Escalado (solo afecta a RFE, no a RF)
            scaler = StandardScaler()
            X_train_scaled = scaler.fit_transform(X_train)
            X_test_scaled = scaler.transform(X_test)

            # === RFE con Logistic Regression (selector) ===
            ...
            rfe = RFE(estimator=lr_base, n_features_to_select=n_features, step=0.1)
            rfe.fit(X_train_scaled, y_train)

            # Transform train/test con las features seleccionadas
