...
for n_features in n_features_to_select_list:
    print(f"\n=== Running SelectKBest (Mutual Information) with {n_features} features ===")

    # Create results directory for this configuration
    results_directory = os.path.join(directory, f'Results/SelectKBest_MI/SelectKBest_{n_features}_features')
    ...
    for g in features:
        ...
        for train_ix, test_ix in outer_cv.split(X, y):
            ...
            # Define pipeline with SelectKBest (Mutual Information) + Random Forest
            pipe = Pipeline([
