...
results_dir = os.path.join(directory, 'Results/Explainability/WPPC')
...
# --- Save CSV ---
top10.to_csv(
    os.path.join(results_dir, 'Top_10_features_WPPC.csv'),
    index=False
)
...
plt.title('Top 10 Feature Importances (WPPC)', fontsize=14)
plt.tight_layout()
plt.savefig(
    os.path.join(results_dir, 'Top_10_Feature_Importance_WPPC.png')
)
plt.close()
...
# ================= SAVE MODEL =================
with open(
    os.path.join(results_dir, 'RF_final_WPPC.pkl'),
    'wb'
) as f:
    pickle.dump(rf_final, f)

print("✅ Explainability analysis for WPPC completed.")
