...
for n_features in n_features_to_select_list:
    ...
    base_results_directory = os.path.join(
        directory, f'Results/final/mRMR/mRMR_{n_features}_features'
    )
    ...
    for max_depth in max_depth_list:
        for min_leaf in min_samples_leaf_list:
            ...
            for g in features:
                ...
                plt.grid(alpha=0.3)
                plt.tight_layout()
                plt.savefig(os.path.join(feature_directory, f"{g}_ROC_curve.png"),
                ...
                plt.grid(alpha=0.3)
                plt.tight_layout()
                plt.savefig(os.path.join(feature_directory, f"{g}_PR_curve.png"),
                            dpi=300, bbox_inches="tight")
                plt.close()
    ...
    print(f"\n✅ Finished mRMR with {n_features} features (all RF combinations)")
