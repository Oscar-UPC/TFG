...
# Number of selected features for RFE
...
for n_features in n_features_to_select_list:
    ...
    print(f"\n=== Running RFE (after ANOVA SelectKBest) with {n_features} features ===")

    results_directory = os.path.join(directory, f'Results/RFE_prefilter_LR/RFE_prefilter_{n_features}_features')
    ...
    for g in features:
        ...
        for C_val in C_values:
            ...
            model_directory = os.path.join(results_directory, g, f"LR_C_{C_val}")
            ...
            for train_ix, test_ix in outer_cv.split(X, y):
                ...
                # === 2) RFE with RANDOM FOREST (corrected) ===
                rfe = RFE(
