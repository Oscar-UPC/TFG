...
for n_features in n_features_to_select_list:
    ...
    base_results_directory = os.path.join(
        directory, f'Results/final/RFE_prefilter/RFE_prefilter_{n_features}_features'
    )
    ...
    for max_depth in max_depth_list:
        for min_leaf in min_samples_leaf_list:
            ...
            for g in features:
                ...
                # Confusion matrix accumulator (sum over folds)
                conf_mat_sum = np.zeros((2, 2))

                # PR baseline like your old script
                baseline_pr = float(np.mean(y))  # prevalence of positive class (MDD)
                ...
                # === ROC FIGURE (old-style: mean curve + luck line) ===
                ...
                # Add mean AUC like your old script text (inside the plot)
                plt.text(0.05, 0.95, f"Mean AUC: {mean_auc:.2f}", transform=plt.gca().transAxes,
                         fontsize=10, va="top")
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(feature_directory, f"{g}_ROC_curve.png"), dpi=300, bbox_inches="tight")
                plt.close()

                # === PR FIGURE (old-style: mean curve + baseline) ===
                ...
                plt.axhline(baseline_pr, linestyle="--", lw=1, label="Baseline (Random Classifier)")

                # Add mean AP like your old script text (inside the plot)
                plt.text(0.05, 0.95, f"Mean AP: {mean_ap:.2f}", transform=plt.gca().transAxes,
                         fontsize=10, va="top")
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(feature_directory, f"{g}_PR_curve.png"), dpi=300, bbox_inches="tight")
                plt.close()

                # === MEAN CONFUSION MATRIX (sum / n_folds_total) ===
                n_folds_total = outer_cv.get_n_splits()
                conf_mat_mean = conf_mat_sum / n_folds_total

                plt.figure(figsize=(5, 4))
                plt.imshow(conf_mat_mean, interpolation="nearest")
                plt.colorbar()
                ...
                for i in range(2):
                    for j in range(2):
                        plt.text(j, i, f"{conf_mat_mean[i, j]:.2f}", ha="center", va="center")
